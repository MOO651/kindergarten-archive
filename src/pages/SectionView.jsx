...
// قائمة افتراضية احتياطية في حال لم يتم العثور على الأقسام المخزنة
...
export default function SectionView() {
  ...
  // جلب اسم القسم ديناميكياً من الـ localStorage (لو الأدمن أضاف قسم جديد سيظهر اسمه هنا فوراً)
  const getSectionTitle = () => {
    try {
      const savedSections = localStorage.getItem('kindergarten_admin_sections');
      if (savedSections) {
        const sectionsList = JSON.parse(savedSections);
        const found = sectionsList.find(sec => String(sec.id) === String(id));
        if (found) return found.title;
      }
    } catch {
      // تجاوز الخطأ في حال حدوثه والاعتماد على القائمة الافتراضية
    }
    return defaultSectionNames[id] || `القسم رقم (${id})`;
  };
