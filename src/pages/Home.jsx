...
export default function Home() {
  ...
  // جلب الأعوام مباشرة من الـ LocalStorage (ولو مفيش بيبدأ بـ 2026 و 2025)
  const [years, setYears] = useState(() => {
    const saved = localStorage.getItem('kindergarten_admin_years');
    return saved ? JSON.parse(saved) : ['2026', '2025'];
  });
  ...
  // تحديث الأعوام والأقسام لو حصل تغيير ورجعت للصفحة
  useEffect(() => {
      ...
      <header className="home-header" style={{ padding: '16px 35px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', position: 'sticky', top: 0, zIndex: 50, background: '#ffffff', borderBottom: '2px solid #e7dfd5' }}>
        <div className="brand-group" style={{ display: 'flex', alignItems: 'center', gap: '15px' }}>
          <div className="brand-logos" aria-label="شعار وزارة التعليم وشعار روضة آل مشول">
            <span className="brand-logo ministry-logo-frame"><img src="/ministry-logo.svg" alt="شعار وزارة التعليم" className="brand-logo ministry-logo" /></span>
            <span className="brand-logo school-logo-frame"><img src="/school-logo.svg" alt="شعار روضة آل مشول" className="brand-logo school-logo" /></span>
          </div>
        ...
        <div style={{ display: 'flex', gap: '12px', alignItems: 'center' }}>
          <div style={{ background: '#f8fafc', padding: '6px 12px', borderRadius: '10px', border: '1px solid #cbd5e1', display: 'flex', alignItems: 'center', gap: '8px' }}>
            ...
            {/* القائمة الآن تعرض الأعوام المخزنة في الـ LocalStorage وتحتوي على ميزة التحديث التلقائي */}
            <select 
              value={selectedYear} 
